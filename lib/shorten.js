...
import { canCreateLink, canCreateAnonymousLink } from "./usage";
import {
  canUseFeature,
  computeExpiresAt,
...
export async function createShortUrl({
  ...
  if (user?.id) {
    ...
    const anonUsage = await canCreateAnonymousLink(ip);
    if (!anonUsage.allowed) {
      return { error: anonUsage.reason, status: 429 };
    }
  }

  const planId = user ? getEffectivePlan(user) : "free";
