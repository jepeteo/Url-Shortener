...
export function getUserFeatures(user) {
  const planId = user?.effectivePlan ?? user?.plan ?? "free";
  const plan = getPlan(planId);

  return {
    customAlias: plan.features.customAlias === true,
    analyticsCharts: plan.features.analyticsCharts === true,
    csvExport: plan.features.csvExport === true,
    apiAccess: plan.features.apiAccess === true,
    configurableExpiry: plan.features.configurableExpiry === true,
  };
}
